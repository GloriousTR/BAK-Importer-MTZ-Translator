...
internal class JsonDisplayLocalizer(private val source: String, private val translate: (String) -> String) {
        ...
        when (source[index]) {
            '{' -> {
                ...
                while (true) {
                    ...
                    value(display || key in DISPLAY_KEYS || key in DISPLAY_CONTAINERS, depth + 1)
                    space()
    ...
    companion object {
        private val DISPLAY_KEYS = setOf(
            "title", "summary", "description", "label", "hint", "text", "subtitle", "message",
        )
        // HyperOS theme metadata stores localized display strings inside these maps.
        private val DISPLAY_CONTAINERS = setOf("titles", "descriptions")
    }
}
