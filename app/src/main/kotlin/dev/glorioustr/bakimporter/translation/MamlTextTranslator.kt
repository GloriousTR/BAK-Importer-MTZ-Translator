...
internal class MamlTextTranslator(
    ...
    private val definitions = elements.filter { element ->
        if (element.tagName != "Var") return@filter false
        val declaredString = element.getAttribute("type").startsWith("string")
        val containsDisplayText = sequenceOf("expression", "values")
            .map(element::getAttribute)
            .any(ThemeGlossary::containsChinese)
        // Many third-party MAML themes omit type="string" even though the variable
        // produces visible text. Chinese literals make that intent unambiguous.
        declaredString || containsDisplayText
    }
    ...
    fun expression(input: String, dateTime: String? = null, depth: Int = 0): String {
        ...
        if (call != null) {
            ...
            return when (call.groupValues[1]) {
                ...
                } else translateKnownDisplayLiterals(s)
    ...
    /** Repairs known visible copy inside malformed third-party ifelse expressions. */
    private fun translateKnownDisplayLiterals(input: String): String {
        var output = input
        listOf("平安吉祥").forEach { source ->
            val translated = translate(source)
            if (translated != source) output = output.replace(quote(source), quote(translated))
        }
        return output
    }
    ...
    private fun dateLiteral(value: String, time: String): String {
        ...
        if (lunarMatch != null && time.trim() == "#time_sys") {
            ...
            // Xiaomi renders N/e as Chinese lunar names (for example 七月廿六).
            // Numeric month/day is language-neutral and compact enough for fixed clock rows.
            val numericDate = if (lunarMatch.value.startsWith("Y")) {
                listOf("#year_lunar", quote("/"), "(1*#month_lunar+1)", quote("/"), "#date_lunar")
            } else {
                listOf("(1*#month_lunar+1)", quote("/"), "#date_lunar")
            }
            return (listOf(dateLiteral(before, time)) + numericDate + dateLiteral(after, time)).joinToString("+")
        }
        ...
        token.findAll(pattern).forEach { match ->
            ...
            output += when {
                ...
                time.trim() == "#time_sys" && match.value.all { it == 'N' } -> "(1*#month_lunar+1)"
