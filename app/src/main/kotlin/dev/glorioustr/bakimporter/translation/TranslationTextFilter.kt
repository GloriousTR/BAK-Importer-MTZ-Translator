...
/** Keeps automatic translation limited to visible, natural-language text. */
object TranslationTextFilter {
    ...
    private val shortDisplayTerms = setOf(
        "music", "play", "pause", "next", "back", "stop", "edit", "save", "done", "more",
        "open", "auto", "on", "off", "yes", "no",
    )
    ...
    fun isCandidate(value: String): Boolean {
        ...
        if (numberOrPunctuation.matches(text)) return false
        val letters = text.count(Char::isLetter)
        if (letters < 2) return false
        if (!text.contains(' ') && text.length <= 5 && text.all { !it.isLetter() || it.isUpperCase() } &&
            text.lowercase() !in shortDisplayTerms
        ) return false
        return true
    }
}
